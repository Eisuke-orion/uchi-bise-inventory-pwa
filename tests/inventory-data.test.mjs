...
test('旧端末へ8月17日の棚卸し値とパインジュースを補完する', () => {
  ...
  assert.equal(result.products.find(product => product.name === '氷').stock, 1);
...
test('8月17日の棚卸し後に入力した牛乳2本は再読込しても維持する', () => {
  ...
  const result = migrateInventoryData(stored(products, {
    ...
    lastUpdated: '2026-08-17T18:00:00+09:00',
  }));
